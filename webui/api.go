	...
	return func(c *gin.Context) {
		if strings.HasPrefix(c.Request.URL.Path, "/webui/api") {
			...
			if c.Param("filepath") == "/api/"+appIDStr+"/api" && c.Request.Method == http.MethodPost {
				...
				// case "get_guild_list":
				// 	// 处理获取群组列表的请求
				// 	handleGetGuildList(c)
				// case "get_channel_list":
				// 	// 处理获取频道列表的请求
				// 	handleGetChannelList(c)
				// case "send_guild_channel_message":
				// 	// 调用处理发送消息的函数
				// 	handleSendGuildChannelMessage(c)
				// default:
				// 	// 处理其他或未知的api名称
...
// func handleGetGuildList(c *gin.Context) {
// 	// 提取前端发来的 pager 数据
